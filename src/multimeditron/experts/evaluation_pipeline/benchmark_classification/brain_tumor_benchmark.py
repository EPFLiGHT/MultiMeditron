...
class BrainTumorBenchmark(ClassificationBenchmark):
    ...
    def __init__(
        ...
        self.train_jsonl = Path(train_jsonl) if train_jsonl is not None else self.default_train_jsonl
        self.test_jsonl = Path(test_jsonl) if test_jsonl is not None else self.default_test_jsonl

    def _read_jsonl(self, path: Path, split_name: str) -> list[dict]:
        examples = []
        dropped_label = 0
        dropped_image = 0
    ...
    def build_train_dataset(self, model, model_name: str, use_cache: bool = True):
        examples = self._read_jsonl(self.train_jsonl, "brain-tumor-train")
