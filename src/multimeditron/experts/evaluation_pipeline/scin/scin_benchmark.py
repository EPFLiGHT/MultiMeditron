...
def _macro_mean_group_score(scores):
    preferred_groups = [
        group for group in ("light", "medium", "dark") if group in scores
    ]
...
class SCINBenchmark(Benchmark):
    ...
    default_eval_jsonls = (
        Path(os.environ["SCIN_EVAL_JSONL"]) if "SCIN_EVAL_JSONL" in os.environ else None,
    )
    default_manifest_jsonls = (
        Path(os.environ["SCIN_MANIFEST_JSONL"]) if "SCIN_MANIFEST_JSONL" in os.environ else None,
    )
    default_protocol_cache_path = (
        REPO_ROOT / "benchmark_cache" / "scin_hard_protocol.json"
    )

    def __init__(
        self,
        eval_jsonls=None,
        manifest_jsonls=None,
        protocol_cache_path=None,
        ref_model_name=REF_MODEL_NAME,
        seed=SEED,
        hard_topk=HARD_TOPK,
    ):
        chosen_eval = (
            eval_jsonls if eval_jsonls is not None else self.default_eval_jsonls
        )
        chosen_manifest = (
            manifest_jsonls
            if manifest_jsonls is not None
            else self.default_manifest_jsonls
        )
        ...
        chosen_cache = (
            protocol_cache_path
            if protocol_cache_path is not None
            else self.default_protocol_cache_path
        )
    ...
    def _validate_paths(self, paths, label):
    ...
    def _load_items(self):
        ...
        logger.info(
            "SCIN skin-tone groups: %s", Counter(item["_skin_group"] for item in items)
        )
        logger.info(
            "SCIN Fitzpatrick labels: %s", Counter(item["_fst"] for item in items)
        )
        return items

    def _load_or_build_protocol(self):
        ...
        logger.info(
            "Building SCIN hard protocol with reference model: %s", self.ref_model_name
        )
        ...
        ref_img_embeds = compute_image_embeds(
            ref_model, ref_processor, device, self.items
        )
    ...
    def evaluate(self, model_path):
        ...
        logger.info(
            "SCIN benchmark score: %.6f | groups=%s", scalar_score, group_scores
        )
        return scalar_score
