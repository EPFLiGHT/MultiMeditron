...
#from transformers import OpenCLIPVisionTextDualEncoderModel
...
class Xray_Dataset(Dataset):
    ...
    def __init__(self, evaluated_clip, saving_name, beginning, end, csv_path):
        ...
        with open(csv_path) as f:
            ...
            file_name_data = os.path.join(embeddings_dir, "data_" + saving_name + ".pt")
            file_name_lab = os.path.join(embeddings_dir, "lab_" + saving_name + ".pt")
            ...
            if os.path.exists(file_name_data):
                ...
                for row in tqdm(csv_file):
                    if i >= beginning and i <= end and i > 0:
                        self.data.append(encode_img(self.evaluated_clip, os.path.join(data_root, "images", row['Image Index'])))
...
class XRay_benchmark(Benchmark):
    ...
    def __init__(self, is_lion_model):
        ...
        _default_data_root = os.path.join(os.path.dirname(__file__), "xray_data")
        self.csv_path = os.path.join(_default_data_root, "Data_Entry_2017_randomized.csv")
    ...
    def evaluate(self, clip_path):
        ...
        return eval_pipeline(clip_path, clip_name, self.sample_number, self.csv_path, self.device)
