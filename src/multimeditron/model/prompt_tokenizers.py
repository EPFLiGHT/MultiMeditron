...
from multimeditron.model.constants import (CONVERSATIONS_KEY,
                                           IGNORE_TOKEN_INDEX, MODALITIES_KEY,
                                           NUM_EMBEDDINGS_KEY, TEXT_KEY)
...
class PromptTokenizer:
    ...
    def tokenize_samples(
        self, samples: Union[List[Dict[str, Any]], Dict[str, Any]], **kwargs
    ) -> List[Dict[str, Any]]:
        """
        ...
        if len(conversations_samples) > 0:
            tokenized_conversations = self.tokenize_conversation(
                conversations_samples, conversations_modalities, **kwargs
            )
    ...
    def update_with_token_range(
        self, tokenized: Dict[str, torch.Tensor], samples: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        processed_samples = []

        for i in range(len(samples)):
            sample = samples[i]
            token_range = self.compute_token_range(
                tokenized["input_ids"][i], sample["modalities"]
            )
    ...
    def pad_tokenized(
        self, tokenized: List[Dict[str, torch.Tensor]]
        ...
        for key in padded_tokenized:
            res[key] = pad_sequence(
                padded_tokenized[key],
    ...
    def tokenize_conversation(
        ...
        res = self._tokenize_conversation(
            prompt,
            modalities,
            add_eos_token=add_eos_token,
            add_generation_prompt=add_generation_prompt,
    ...
    def tokenize_text(
        self, prompt: List[str], modalities: List[List[Dict[str, Any]]]
    ) -> List[Dict[str, Any]]:
    ...
    def get_num_embeddings(self, modality: Dict[str, Any]) -> int:
        ...
        if (
            modality["type"] in self.modalities_num_embeddings
            and self.modalities_num_embeddings[modality["type"]] is not None
        ):
            return self.modalities_num_embeddings[modality["type"]]

        raise ValueError(
            f"Modality should contain a {NUM_EMBEDDINGS_KEY} key or \
                         you should give a num_embeddings for {modality['type']} to this PromptTokenizer"
        )

    def compute_token_range(
        self,
        sequence_input_ids: torch.Tensor,
        sequence_modalities: List[Dict[str, Any]],
    ) -> List[Tuple[int, int]]:
        """
        ...
        modalities_indices = torch.argwhere(
            sequence_input_ids == self.attachment_token_idx
        ).flatten()
        modalities_lengths = [self.get_num_embeddings(m) for m in sequence_modalities]

        modalities_token_start = modalities_indices[
            np.cumsum([0] + modalities_lengths[:-1])
        ].tolist()
        modalities_token_range = [
            (start, start + length)
            for start, length in zip(modalities_token_start, modalities_lengths)
        ]
    ...
    def expand_attachment_input_tokens(
        ...
        modalities_for_message: List[Dict[str, Any]],
        ...
        modalities_indices = torch.argwhere(
            token_ids == self.attachment_token_idx
        ).flatten()
        ...
        for previous_mod_idx, current_mod_idx, mod in zip(
            ...
            # Add the text
            expanded_token_ids.append(token_ids[previous_mod_idx + 1 : current_mod_idx])
    ...
    def _tokenize_text(
        self, text: List[str], modalities: List[List[Dict[str, Any]]]
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(len(text)):
            ...
            labels = torch.where(
                input_ids == self.attachment_token_idx, self.ignore_index, input_ids
            )
