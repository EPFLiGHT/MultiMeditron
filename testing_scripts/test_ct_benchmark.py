...
from evaluation_pipeline.benchmark_classification.ct_benchmark import CTBenchmark  # noqa: E402
...
class SmokeCTBenchmark(CTBenchmark):
    def __init__(self, subset_fraction: float, cache_root: Path | None = None) -> None:
    ...
    def load_examples(self) -> list[dict]:
        ...
        subset_size = max(1, int(self.subset_fraction * len(examples)))
        return examples[:subset_size]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Smoke test for the CT benchmark image loading and dataset building.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0001,
        help="Fraction of CT examples to use for the smoke test before the train/test split.",
    )
    parser.add_argument(
        "--cache-root",
        type=Path,
        default=None,
        help="Optional cache directory for precomputed CT embeddings.",
    )
    parser.add_argument(
        "--log-dir",
        type=Path,
...
def main() -> None:
    ...
    log_path = setup_logging(args.log_dir, "ct")
    ...
    benchmark = SmokeCTBenchmark(subset_fraction=args.subset_fraction, cache_root=args.cache_root)
    use_cache = not args.rebuild_cache
    model_name = args.model_path.name

    print(f"Testing CTBenchmark with model: {args.model_path}")
    ...
    if args.full_eval:
        ...
        print(f"CT benchmark result: {result}")
